...
$sql_year = "SELECT YEAR(record_date) as yr, SUM(revenue) as total FROM store_daily_stats WHERE store_id = $current_id GROUP BY YEAR(record_date) ORDER BY yr ASC";
...
$sql_month = "SELECT MONTH(record_date) as mon, SUM(revenue) as total FROM store_daily_stats WHERE store_id = $current_id GROUP BY MONTH(record_date) ORDER BY mon ASC";
...
$sql_order_day = "SELECT record_date, order_count FROM store_daily_stats WHERE store_id = $current_id ORDER BY record_date DESC LIMIT 7";
...
$sql_order_wk = "SELECT WEEK(record_date) as wk, SUM(order_count) as total FROM store_daily_stats WHERE store_id = $current_id GROUP BY WEEK(record_date) ORDER BY wk ASC";
...
$sql_user = "SELECT YEAR(record_date) as yr, SUM(new_users) as total FROM store_daily_stats WHERE store_id = $current_id GROUP BY YEAR(record_date) ORDER BY yr ASC";
...
// 5. 今日即時營運指標數據
// ==========================================================================
$sql_today = "SELECT revenue, order_count FROM store_daily_stats WHERE store_id = $current_id AND record_date = CURDATE()";
...
<head>
  ...
  <style>
    ...
    .data-container {
      max-width: 1280px; margin: 90px auto 40px; padding: 0 20px; box-sizing: border-box;
    }
  ...
  <script type="text/javascript">
    ...
    // 🟢 修正處：稍微延遲 200 毫秒後再執行繪製，能讓網頁載入時的入場動畫（Animation）更流暢更明顯
    google.charts.setOnLoadCallback(() => {
      setTimeout(drawAllCharts, 200);
    });

    const salesData = {
      year: <?php echo json_encode($sales_year_data); ?>,
      month: <?php echo json_encode($sales_month_data); ?>,
      week: <?php echo json_encode($sales_week_data); ?>
    };
    ...
    function drawAllCharts() {
      ...
      // 🟢 核心修正：定義 Google Charts 的原生進場動畫參數（時間 1.2 秒，由下往上延伸滑出）
      ...
      if(salesStage) {
        const salesChart = new google.visualization.LineChart(salesStage);
        salesChart.draw(google.visualization.arrayToDataTable(salesData[currentSalesType]), {
      ...
      if(orderStage) {
        const orderChart = new google.visualization.ColumnChart(orderStage);
        orderChart.draw(google.visualization.arrayToDataTable(orderData[currentOrderType]), {
      ...
      if(userStage) {
        const userChart = new google.visualization.LineChart(userStage);
        userChart.draw(google.visualization.arrayToDataTable(<?php echo json_encode($user_growth_data); ?>), {
          colors: ['#4e8f70'], fontName: 'Noto Sans TC', chartArea: { width: '85%', height: '70%' },
          legend: { position: 'none' }, lineWidth: 3, pointsVisible: true, hAxis: { textStyle: textStyle }, vAxis: { textStyle: textStyle },
          animation: chartAnimation // 🟢 注入動畫
        });
      }

      // --- 4. 熱銷剩食比例（圓餅圖原生不支援 startup 延伸動畫，但有淡入特效）
      const pieStage = document.getElementById('pie_chart_div');
      if(pieStage) {
        const pieChart = new google.visualization.PieChart(pieStage);
        pieChart.draw(google.visualization.arrayToDataTable([['項目', '份數'], ['勁辣雞腿堡套餐', 340], ['大薯分享盒', 220], ['雙層牛肉吉事堡', 180]]), {
          colors: ['#6cae8b', '#8ab8a1', '#a3c9b6'], fontName: 'Noto Sans TC', chartArea: { width: '90%', height: '75%' },
    ...
    function openModal(chartType, titleText) {
      ...
      setTimeout(() => {
        ...
        if (chartType === 'sales') {
          ...
          chart.draw(google.visualization.arrayToDataTable(salesData[currentSalesType]), {
          ...
          const chart = new google.visualization.LineChart(stage);
          chart.draw(google.visualization.arrayToDataTable(<?php echo json_encode($user_growth_data); ?>), {
            colors: ['#4e8f70'], chartArea: { width: '90%', height: '80%' }, lineWidth: 4, pointsVisible: true, pointSize: 6, legend: { position: 'none' }
          });
        } else if (chartType === 'pie') {
          const chart = new google.visualization.PieChart(stage);
          chart.draw(google.visualization.arrayToDataTable([['項目', '份數'], ['勁辣雞腿堡套餐', 340], ['大薯分享盒', 220], ['雙層牛肉吉事堡', 180]]), {
            colors: ['#6cae8b', '#8ab8a1', '#a3c9b6', '#c2ded0'], chartArea: { width: '85%', height: '80%' }, legend: { position: 'right', textStyle: { fontSize: 14 } }, is3D: true
          });
        }
      }, 150);
    }
    ...
    <div class="menu-content">
      ...
      <div>
        ...
        <div class="menu-list">
          ...
          <a href="seller-data.php" class="menu-item active">📈 數據中心</a>
