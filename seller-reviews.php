...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'reply') {
    ...
    $sql_update = "UPDATE store_reviews SET 
                  ...
                  reply_date = NOW() 
                  WHERE No = $review_id AND store_id = $current_id";
...
<head>
  ...
  <style>
    ...
    /* 內容主容器 */
    .reviews-container {
      max-width: 900px; margin: 90px auto 40px; padding: 0 20px; box-sizing: border-box;
    }
    ...
    .reviews-scroll-box::-webkit-scrollbar-thumb { background: var(--green-pale); border-radius: 10px; }
    .reviews-scroll-box::-webkit-scrollbar-thumb:hover { background: var(--green-deep); }
    ...
    .user-avatar { width: 48px; height: 48px; border-radius: 50%; object-fit: cover; border: 2px solid var(--cream); }
    .user-info { flex-grow: 1; }
    .user-meta { display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; }
    .user-name { font-size: 1.5rem; font-weight: 700; color: var(--text-dark); }
    .review-date { font-size: 1.1rem; color: var(--text-muted); }
    
    .stars-row { color: #dba13a; font-size: 1.3rem; }
    .comment-text { font-size: 1.45rem; line-height: 1.6; color: var(--text-dark); margin: 8px 0 4px 0; }
    ...
    .seller-reply-box {
      ...
      padding: 14px; border-radius: 10px; border-left: 4px solid var(--green-deep);
      position: relative;
    }
    ...
    .reply-text-display { font-size: 1.35rem; line-height: 1.5; color: var(--text-dark); word-break: break-all; }

    /* 🟢 修改按鈕樣式 */
    .edit-reply-btn {
      position: absolute; right: 14px; top: 14px; background: transparent;
      border: 1px solid var(--green-pale); color: var(--text-muted); padding: 4px 10px;
      border-radius: 6px; cursor: pointer; font-size: 1.1rem; transition: all 0.2s;
    }
    .edit-reply-btn:hover { background: var(--green-deep); color: white; border-color: var(--green-deep); }

    /* 整合式輸入表單 */
    .reply-form { display: flex; flex-direction: column; gap: 10px; width: 100%; }
    .input-wrapper {
      display: flex; align-items: center; background: var(--white); border: 1px solid var(--green-pale);
    ...
    .submit-btn {
      background: var(--green-deep); color: white; border: none; border-radius: 50%;
    ...
    <div class="menu-content">
      ...
      <div>
        ...
        <div class="menu-list">
          ...
          <a href="seller-reviews.php" class="menu-item active">💬 評論管理</a>
              ...
              <div class="seller-reply-box" id="display-box-<?php echo $review['No']; ?>">
                ...
                <div class="reply-content">
                  <div class="reply-meta">
                    <span style="color:var(--green-deep);">商家回覆</span>
                    <span style="color:var(--text-muted); font-weight:normal; font-size:1.1rem;">
